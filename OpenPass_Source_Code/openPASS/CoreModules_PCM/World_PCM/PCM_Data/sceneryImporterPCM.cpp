...
SceneryImporterPCM::SceneryImporterPCM(const CallbackInterface *callbacks):
    callbacks(callbacks)
{}
                                ...
                                std::map<int, Trajectory> &trajectories)
{
    ...
    if (!xmlFile.open(QIODevice::ReadOnly)) {
        LOG(CbkLogLevel::Warning, "an error occurred during scenery import");
        return false;
    }
    ...
    if (!document.setContent(xmlData, &errorMsg, &errorLine)) {
        ...
        log << "invalid xml file format of file " << filename;
        LOG(CbkLogLevel::Warning, log.str());
        log << "in line " << errorLine << " : " << errorMsg.toStdString();
        LOG(CbkLogLevel::Warning, log.str());
        return false;
    }
