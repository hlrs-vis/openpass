...
PCM_Point PCM_LineSegment::GetNearestPointFromPoint(const PCM_Point *point, double viewDirection,
                                                    double range) const
{
    ...
    if (std::isinf(
                viewDirection)) { // check if viewing direction is infinity --> no specific viewing direction
        ...
        nearestPoint = CalcIntersectionPointInDirection(point, viewDirection);
        ...
        nearestPoint = CalcNearestPointFromPointInViewRange(point, viewDirection, range);
    }

    return nearestPoint;
}

double PCM_LineSegment::CalcDistanceFromPoint(const PCM_Point *point, double viewDirection,
                                              double range) const
{
    if (!CheckAllPointsValid(point)) {
        return INFINITY;
    ...
    PCM_Point nearestPoint = GetNearestPointFromPoint(point, viewDirection, range);
...
PCM_Point PCM_LineSegment::CalcNearestPointFromPoint(const PCM_Point *point) const
{
    ...
    // rotate so lineSegVector is on the X-axis in positive viewDirection with
    // first point in (0|0)
                                                                ...
                                                                double viewDirection, double range) const
{
    ...
    if ((std::isinf(viewDirection) || (std::isinf(range)))) {
        return PCM_Point();
    }
